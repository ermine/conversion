...
let decoder_path = ref ""
let encoder_path = ref ""
    ...
    let w = Hashtbl.find decoders name in
	    match Weak.get w 0 with
	      ...
	      | None ->
		        ...
		        raise Not_found
  with Not_found ->
    let file = Filename.concat !decoder_path (name ^ ".dec") in
    let bin = 
    ...
    let w = Hashtbl.find encoders name in
	    match Weak.get w 0 with
	      ...
	      | None ->
		        ...
		        raise Not_found
  with Not_found ->
    let file = Filename.concat !encoder_path (name ^ ".enc") in
    let bin = 
	    try open_in_bin file with Sys_error _ -> raise (UnknownEncoding name) in
    let mar = input_value bin in
    let w = Weak.create 1 in
